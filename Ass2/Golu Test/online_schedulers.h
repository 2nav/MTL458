...
void push(Queue *queue, Process *process)
{
    ...
    {
        printf("Cannot push more elements!\n");
        return;
    }
...
// waiting for child to finish, then check status
...
// run for quantum when seeing process for first time
// run to completion when seeing process for first time if [bool to_completion]
// quantum does not matter when to_completion is true
...
bool all_empty(Queue *queues[], int num_queues) // MLFQ all 3 queues are empty
{
...
// preserves relative order within each queue. pourses hogher priority queues to queue 0 first
...
// remove spaces and concatenate. Used to compare commands
char *generate_command_hash(const char command[])
{
    size_t char_cnt = 0;
...
// when a process exits, the expercted burst time for all other processes with the same command hash is updated
